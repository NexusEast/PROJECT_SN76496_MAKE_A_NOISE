...
SD_Error SDEnWideBus(u8 enx)
{
	...
	printf("FindSCR result:%d\r\n",errorstatus);
	printf("SDEnWideBus 1371 \r\n");
...
{ 
	...
	{ 
		FlagStatus stats = SDIO_GetFlagStatus(SDIO_FLAG_RXDAVL);
		
		printf("SDIO_GetFlagStatus:%d,SDIO->STA:%d\r\r\n",stats,SDIO->STA);
		if(stats != RESET)//接收FIFO数据可用
		{
			printf("reading data,index:%d\r\r\n",index);
